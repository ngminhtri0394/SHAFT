...
from env.graph_crystal_env import HierGraphCrystalEnv, HierGraphMEGNetCrystalEnv, HierGraphGCNCrystalEnv
from pl_modules.proxy import M3gnetDGL_Proxy
import torch
# torch.set_printoptions(profile="full")
from policy.spacegroup_policy import GCNSpaceGroupPolicy, BWGCNSpaceGroupPolicy, \
                                    MLPSpaceGroupPolicy, BWMLPSpaceGroupPolicy, \
                                    SpaceGroupPolicy, BWSpaceGroupPolicy
from policy.atom_lattice_policy import GCNLatticeAtomPolicy, BWGCNLatticeAtomPolicy,\
                                    MLPLatticeAtomPolicy,BWMLPLatticeAtomPolicy, \
                                    LatticeAtomPolicy, BWLatticeAtomPolicy
...
class SHAFT():
    ...
    def init_env(self):
        if self.policy_nn == 'graph_gcn':
            self.env = HierGraphGCNCrystalEnv(device=self.device, pretrain_model=self.pretrain_model_name, ele_set=self.ele_set,batch_size=self.batch_size)
    ...
    def init_policy(self, path=None):
        if self.policy_nn == 'graph_gcn':
            sgbackbone = GCNBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.sgpolicy = SpaceGroupPolicy(backbone=sgbackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim).to(device=self.device)
            labackbone = GCNBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.latticeatompolicy = LatticeAtomPolicy(backbone=labackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim,
                                            n_element=self.num_elementset).to(device=self.device)
            bwsgbackbone = GCNBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.bwsgpolicy = BWSpaceGroupPolicy(backbone=bwsgbackbone,use_pretrain=self.use_pretrain,
                                                    hidden_dim=self.phidden_dim,
                                                    pretrain_dim=self.pretrain_dim).to(device=self.device)
            bwlabackbone = GCNBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.bwlatticeatompolicy = BWLatticeAtomPolicy(backbone=bwlabackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim,
                                            n_element=self.num_elementset).to(device=self.device)
        if self.policy_nn == 'graph_megnet':
            sgbackbone = MEGNetBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.sgpolicy = SpaceGroupPolicy(backbone=sgbackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim,batchsize=self.batch_size).to(device=self.device)
            labackbone = MEGNetBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.latticeatompolicy = LatticeAtomPolicy(backbone=labackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
            ...
            bwsgbackbone = MEGNetBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.bwsgpolicy = BWSpaceGroupPolicy(backbone=bwsgbackbone,use_pretrain=self.use_pretrain,
                                                    hidden_dim=self.phidden_dim,
                                                    pretrain_dim=self.pretrain_dim,batchsize=self.batch_size).to(device=self.device)
            bwlabackbone = MEGNetBackbone(use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
                                            pretrain_dim=self.pretrain_dim)
            self.bwlatticeatompolicy = BWLatticeAtomPolicy(backbone=bwlabackbone,use_pretrain=self.use_pretrain,
                                            hidden_dim=self.phidden_dim,
        ...
        if path != None:
            if self.policy_nn == 'graph_dimenetpp' or self.policy_nn == 'graph_megnet':
                self.sgpolicy.load_state_dict(torch.load(f'{path}/'+'saved_weight_to_sample/policy_fp_sg.pt')['model_state_dict'])
                self.latticeatompolicy.load_state_dict(torch.load(f'{path}/'+'saved_weight_to_sample/policy_fp_alt.pt')['model_state_dict'])
                self.bwsgpolicy.load_state_dict(torch.load(f'{path}/'+'saved_weight_to_sample/policy_bw_sg.pt')['model_state_dict'])
                self.bwlatticeatompolicy.load_state_dict(torch.load(f'{path}/'+'saved_weight_to_sample/policy_bw_alt.pt')['model_state_dict'])
            else:
                self.sgpolicy = torch.load(f'{path}/'+'saved_weight_to_sample/policy_fp_sg.pt')
                self.latticeatompolicy = torch.load(f'{path}/'+'saved_weight_to_sample/policy_fp_alt.pt')
                self.bwsgpolicy = torch.load(f'{path}/'+'saved_weight_to_sample/policy_bw_sg.pt')
                self.bwlatticeatompolicy = torch.load(f'{path}/'+'saved_weight_to_sample/policy_bw_alt.pt')
    ...
    def save_info(self,iter):
        ...
        if self.policy_nn == 'graph_megnet':
            torch.save({'model_state_dict': self.hpolicylist[0].state_dict(),
                    }, f'{exp_dir}/'+'policy_fp_sg.pt')
            torch.save({'model_state_dict': self.hpolicylist[1].state_dict(),
                    }, f'{exp_dir}/'+'policy_fp_alt.pt')
            torch.save({'model_state_dict': self.bwhpolicylist[0].state_dict(),
                    }, f'{exp_dir}/'+'policy_bw_sg.pt')
            torch.save({'model_state_dict': self.bwhpolicylist[1].state_dict(),
                    }, f'{exp_dir}/'+'policy_bw_alt.pt')
        else:
    ...
    def train_model_with_proxy(self):
        ...
        for i in tr:
            ...
            logrewards = self.reward_beta * torch.Tensor(reward).to(device=self.device).log()
